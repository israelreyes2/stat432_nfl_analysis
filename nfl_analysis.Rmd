...
GBM model gives us a trainning rmse of `r gbm_RMSE` . This value was the best RMSE from the rest of the models, so we will advance our study further using this model. 
...
We all that in our mind, a further study utilizing both offensive and defensive statistics of the game will be suggested. In an attempt to analytically breakdown football, we realized that it might be harder than analyze other games such as baseball due to so many unpredictable situations in the game. 
