...
```{r variable clean up, offense only, include = FALSE}
...
```{r, checking no_plays in play type, include = FALSE}
#no_play = subset(df_2018, df_2018$play_type == "no_play")
#head(no_play)
```

```{r, removing adminstrative playtypes, include = FALSE}
...
```{r, removing qb kneels, include = FALSE}
...
```{r replacing NAs with characters, include = FALSE}
...
```{r variable factor type conversion, include = FALSE}
...
```{r calculating rmse, include = FALSE}
calc_rmse = function(actual, predicted){
  sqrt(mean((actual - predicted)^2))
}
```

```{r ridge modeling, include = FALSE}
