...
```{r}
linear_model = lm(est_y ~ est_x)
linear_pred = predict(linear_model, as.data.frame(val_x))
linear_rmse = calc_rmse(linear_pred, val_y)
```

# Results
```{r}
results = tibble(
  Model = c("Lasso", "Ridge", "GBM", "Linear"),
  "Validation RMSE" = c(lasso_rmse, ridge_model_RMSE, gbm_RMSE, linear_rmse)) %>%
  kable(digits = 3) %>%
  kable_styling("striped", full_width = FALSE)
```

```{r test rmse, cache = TRUE}
# Omitting random NAs in the data
nfl_trn = na.omit(nfl_trn)
