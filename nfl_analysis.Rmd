...
```{r, ridge modeling}
ridge_data_x = df_2018_cleaned %>% select(-wpa)
ridge_data_y = df_2018_cleaned %>% pull(wpa)

ridge_mod  = glance.cv.glmnet(x = x_ridge, y = y_ridge, alpha = 0)

plot(ridged_mod, xvar = "lambda", label = TRUE)
grid()
```
